                ...
                if path.exists(users_jsons+"\\"+usr["initial"]+"\\SS"):
                    for file in listdir(users_jsons+"\\"+usr["initial"]+"\\SS"):
                        if file.startswith("Action") and file.endswith(".json"):
                            ...
                            if datetime.strptime(data["ClosedDate"],"%d/%m/%Y %H:%M")+timedelta(days=3)<=datetime.now():
        ...
        for pageNo in range(doc.pageCount):  #for each page no in the pdf
            ...
            for RECT in page.annots(): # for each annots/xrefs/textboxes in the page
                ...
                if dictkey in textBoxDict:
                    ...
                    fontfile = gadugi_fontfile,  #specify font, gadugi_fontfile is a variable which is set as the downloaded gadugi path
                    align = text_align)                      
...
class ProjectsWidget(QWidget):
    def __init__(self):
        try:
          ...
          #Connect the table item click and doubleclick to their functions
            ...
            projectsTable.doubleClicked.connect(self.on_projectDblclick)
          ...
          #Sizing and looks of the project table
            ...
            colsizelist=[projectsTable.horizontalHeader().sectionSize(0),projectsTable.horizontalHeader().sectionSize(1),projectsTable.horizontalHeader().sectionSize(2)]
            projectsTable.setSelectionBehavior(1) #Set the selection mode of the cells
            for i in range(0,3):
                header.setSectionResizeMode(i, QHeaderView.Interactive)
                projectsTable.setColumnWidth(i,colsizelist[i]+50)
          ...
          #Projects Info
            ...
            self.next_button.clicked.connect(lambda: self.changeSlctdProject(1))
            self.style_slctdProjectNameLabel()
            ...
            projectsInfoLayout.addSpacing(40)
            projectsInfoLayout.addLayout(slctdProjectLabelLayout, Qt.AlignCenter)
    ...
    def style_slctdProjectNameLabel(self):
        try:
            ...
            self.previous_button.setFixedSize(50,32)
            self.next_button.setFixedSize(50,32)
            #change buttons style
            self.previous_button.setStyleSheet("background:white; border: none;")
            self.next_button.setStyleSheet("background:white; border: none;")
            #hover effect
...
class ActionsTable(QTableWidget):
    def __init__(self):
        try:
            ...
            normalboldfont=QFont()
            # normalboldfont.setBold(True)
            normalboldfont.setPointSize(9)
            normalboldfont.setFamily("Gadugi")
            ...
            for usr_actions in actions:
                    ...
                    for d in range(len(usr_actions)): # for each actions
                        ...
                        if "WeeklyData" in usr_actions[d]: self.item(self.rowCount()-len(usr_actions)+d,self.columnLabels.index("Action")).setData(256,usr_actions[d]["WeeklyData"])
                        ...
                        if self.item(self.rowCount()-len(usr_actions)+d,self.columnLabels.index("Action")).data(256) not in [None, {}]: #if there's a weekly data
                            for wk, wkdata in self.item(self.rowCount()-len(usr_actions)+d,self.columnLabels.index("Action")).data(256).items():
                                weeklydata+= f"{wk} : {wkdata['notes']} | {wkdata['hrs']}hrs\n"
                ...
                if len(selectedrows)>0:
                    ...
                    if this_userdata!=None and all([self.item(i,self.columnLabels.index("For")).text()==this_userdata["initial"] for i in selectedrows]): #if all selected actions are for this user
                        ...
                        if all([self.item(i,self.columnLabels.index("Status")).text()=="Open" for i in selectedrows]):
                            ...
                            #assign to week
                            weekassignaction=QAction("Assign to Week")
                            weekassignaction.triggered.connect(self.weekAssignAction)
                            menu.addAction(weekassignaction)
                            #complete action
                            completeaction=QAction("Complete")
                            completeaction.triggered.connect(self.completeAction)
                        ...
                        #recover action - if all selected rows actions are completed
                            ...
                            recoveraction=QAction("Recover")
                            recoveraction.triggered.connect(self.recoverAction)
                        ...
                        #approve action - if all actions are not approved
                            ...
                            approveAction = QAction(QIcon(tickdone_icon2),"Approve")
                            approveAction.triggered.connect(self.approveAction)
                            menu.addAction(approveAction)
                        #delete action
    ...
    def editAction(self):
        try:
            ...
            if len(selectedrows)==1 and self.item(selectedrows[0],self.columnLabels.index("Status")).text()=="Open" and (this_userdata!=None and this_userdata["initial"]==self.item(selectedrows[0],self.columnLabels.index("For")).text()):
                ...
                def saveChanges():
                    if dialog.weeksData=={} and len(dialog.WeekBox.checkedItems)==1:
                        newWeekData= {dialog.WeekBox.itemData(next(iter(dialog.WeekBox.checkedItems))).toString("dd/MM/yyyy"):{'hrs': dialog.HoursRequiredBox.value()+dialog.MinutesRequiredBox.value()/60,'notes':""}}
                    else:    
                        newWeekData= {dateObj.toString("dd/MM/yyyy"):{'hrs': data['spinbox'].value(),'notes':data['textedit'].toPlainText()} for dateObj,data in dialog.weeksData.items()}
                ...
                if foundWeeks!=None:
                    ...
                    for wk in foundWeeks:
                        if QDate.fromString(wk,'dd/MM/yyyy')<QDate.currentDate().addDays(-QDate.currentDate().dayOfWeek() + 1):
                            ...
                            spinbox.setValue(foundWeeks[wk]['hrs'])
                            textedit=QTextEdit()
                            textedit.setText(foundWeeks[wk]['notes'])
                            dialog.weeksData[start]={'label':f"{start.toString('MMM dd')} - {end.toString('MMM dd')}", 'spinbox':spinbox,'textedit':textedit}
                ...
                for count in range(30):
                    ...
                    if foundWeeks and startToString in foundWeeks:
                        ...
                        dialog.WeekBox.checkedItems.add(index)
                        spinbox=QDoubleSpinBox()
                        spinbox.setValue(foundWeeks[startToString]['hrs'])
                        textedit=QTextEdit()
    ...
    def WeekChecked(self,dialog):
        try:
            if self.WeekCheck.isChecked():
                ...
                dialog.WeekBox.setEnabled(False)
                #uncheck all weeks
                for count in range(dialog.WeekBox.count()):
                    dialog.WeekBox.model().item(count).setCheckState(Qt.Unchecked)
                dialog.WeekBox.checkedItems=set()
                dialog.WeekBox.hidePopup()
        except:
            MsgBox(str(format_exc())+"\n\n Contact software programmer", setWindowTitle="Error", setIcon = QMessageBox.Critical)
    ...
    def weeklyReport(self, actionfor, selectedWeekPair, actionRows=None):
        try:
            ...
            def removeRow(gridrow, tablerow):
                for i in reversed(range(grid.columnCount())):
                    grid.itemAtPosition(gridrow,i).widget().deleteLater()
                removedRows.append(tablerow)
            def clearWeek():
                for i in reversed(range(grid.rowCount())):
                    for j in reversed(range(grid.columnCount())):
                        if grid.itemAtPosition(i,j) and grid.itemAtPosition(i,j).widget() and grid.itemAtPosition(i,j).widget().__class__==QPushButton and grid.itemAtPosition(i,j).widget().text()=="Remove":
                            grid.itemAtPosition(i,j).widget().clicked.emit()
                            break
            def saveChanges(selectedWeek):
                ...
                for r in removedRows:
                    if self.item(r,self.columnLabels.index("Action")).data(256)!=None:
                        ...
                        if selectedWeek in weekdata:
                            del weekdata[selectedWeek]
                            rowsToEdit[r]=weekdata
                #check added rows
                for r in selectedActions:
                    if r not in removedRows: 
                        if self.item(r,self.columnLabels.index("Action")).data(256)!=None:
                            weekdata= self.item(r,self.columnLabels.index("Action")).data(256)
                            if selectedWeek not in weekdata or (weekdata[selectedWeek]['hrs']!=selectedActions[r]['hrs'].value() or weekdata[selectedWeek]['notes']!=selectedActions[r]['notes'].toPlainText()):
                                weekdata[selectedWeek]={'hrs': selectedActions[r]['hrs'].value(),'notes':selectedActions[r]['notes'].toPlainText()}
                                rowsToEdit[r]=weekdata
                        else:
                            weekdata[selectedWeek]={'hrs': selectedActions[r]['hrs'].value(),'notes':selectedActions[r]['notes'].toPlainText()}
                            rowsToEdit[r]=weekdata
                if rowsToEdit:
                    for r, weekdata in rowsToEdit.items():
          ...
          #get actions, project, filename and week data for the week selected
            for r in actionRows:
                ...
                foundWeeks= self.item(r,self.columnLabels.index("Action")).data(256)
                selectedActions[r]["hrs"]= QDoubleSpinBox()
                ...
                if foundWeeks!=None and selectedWeek in foundWeeks:
                    selectedActions[r]['hrs'].setValue(foundWeeks[selectedWeek]['hrs'])
                    selectedActions[r]["notes"].setPlainText(foundWeeks[selectedWeek]['notes'])
                else:
                    hrs= self.item(r,self.columnLabels.index("Time Required")).data(256)['hr']+self.item(r,self.columnLabels.index("Time Required")).data(256)['min']/60
                    selectedActions[r]['hrs'].setValue(hrs)
          ...
          #add actions to grid
            row=0
            for r in selectedActions:
                actionDetails=QLabel(f"{selectedActions[r]['Action']} \n({selectedActions[r]['Project'].split(' - ',1)[1]})")
                actionDetails.setStyleSheet("background:white; border: none; color:rgba(0,0,0,0.9); font-size: 15px;")
                actionDetails.setWordWrap(True)
                grid.addWidget(actionDetails,row,0,1,2)
                grid.addWidget(selectedActions[r]["hrs"],row,2)
                grid.addWidget(selectedActions[r]["notes"],row,3)
          ...
          #Main layout
            ...
            inputsScroll.setWidgetResizable(True)
            mainlayout=QVBoxLayout()
            topWeekLabel=QLabel(list(selectedWeekPair.keys())[0])
            topWeekLabel.setStyleSheet("font-size:17px; font-weight:bold;padding-left:10px; padding-right:10px; background-color:rgba(255,255,255,0.9);color:rgba(0, 0, 0,1);")
...
class ActionsWidget(QWidget):
    def __init__(self):
        try:
          ...
          #Week box
            self.previous_button = QToolButton()
            self.next_button = QToolButton()
            self.previous_button.setIcon(QIcon(previous_icon))
            self.next_button.setIcon(QIcon(next_icon))
            self.previous_button.setIconSize(QSize(21, 21))
            self.next_button.setIconSize(QSize(21, 21))
            ...
            # self.active_week = QDate.currentDate()
            self.week_label = QPushButton()
            self.week_label.setIcon(QIcon(calendar_icon))
            self.week_label.setIconSize(QSize(18,17))
            self.week_label.setEnabled(False)
            self.week_label.setStyleSheet("background:white; border: none; color:rgba(0,0,0,0.7); font-size: 15px;")
            self.week_label.setFixedSize(170, 32)
            self.update_week_label()
            week_layout = QHBoxLayout()
            week_layout.addWidget(self.previous_button)
            week_layout.addWidget(self.week_label, alignment=Qt.AlignCenter)
            week_layout.addWidget(self.next_button)
          ...
          #Action Table
            ...
            projectwindow_actiontable=self.actionTable.__class__()
            #Connecting the week overview button to its function when clicked
            self.WeekOverviewButton.clicked.connect(lambda: self.actionTable.weeklyReport(self.actionsUserBox.currentText(),{self.week_label.text():self.active_week.toString("dd/MM/yyyy")}))
          ...
          #Page Layouts
            ...
            toprightlayout.addLayout(week_layout)
            # toprightlayout.addStretch(1)
            # toprightlayout.addWidget(self.actionLabel)#, alignment=Qt.AlignCenter)
            toprightlayout.addStretch(1)
            self.MainLayout.addLayout(toprightlayout, 1, 1, Qt.AlignLeft)
            toprightlayout.addStretch(2)
    ...
    def update_week_label(self):
        ...
        if QDate.currentDate().addDays(-QDate.currentDate().dayOfWeek()+1)==start_date:
            week_text ="This week"
        elif QDate.currentDate().addDays(-QDate.currentDate().dayOfWeek()+1).addDays(-7)==start_date:
            week_text ="Last week"
        else:
        ...
        try:
            ...
            if actionfor=="All users" or (actionfor=="All users" and project!=None):
                ...
                if project=="All projects" or project==None:
                    ...
                    color_dict = {proj: cmap((persons.size-1-i) % cmap.N)  for i, proj in enumerate(persons)}
                    df['color'] = df['ActionFor'].map(color_dict)
                    ...
                    # legend_elements = [Patch(facecolor=color, label=proj) for proj, color in color_dict.items()]
                    ...
                    for idv, row in df.iterrows():
                        ...
                        leg = self.ax.legend(leg_actions, loc='upper center', bbox_to_anchor=(0.5, -0.5),ncol=1, fontsize=8)
                ...
                for acts in self.actions:
                    if acts !=[] and acts[0]["ActionFor"]==actionfor and list(filter(lambda i: i['Status']=="Open", acts)):
                        ...
                        if actions==[]:
                            ...
                            # Assign colors to the 'color' column in the DataFrame
                            df['color'] = df['Project'].map(color_dict)


                            ##Old random colors
                            # color_dict = {}
                            # # Generate a new random color for each row
                            # df['color'] = df.apply(lambda x:  color_dict.setdefault(x['Project'],'#' + '%02x%02x%02x' % tuple(int(255 * i) for i in hls_to_rgb(np_random.rand(), 0.5, np_random.uniform(0.5, 1)))), axis=1)


                            # # color_dict[row['Project']] = plt.get_cmap('tab20')(len(color_dict))
...
class NewAction_Dialog(QDialog):
    ...
    def WeekChecked(self):
        try:
            if self.WeekCheck.isChecked():
                ...
                self.WeekBox.setEnabled(False)
                #uncheck all weeks
                for count in range(self.WeekBox.count()):
                    self.WeekBox.model().item(count).setCheckState(Qt.Unchecked)
                self.WeekBox.checkedItems=set()
    ...
    def checkBoxhidePop(weeksData, weeksGrid, WeekBox, HoursRequiredBox, MinutesRequiredBox):
        ...
        for datObj in list(weeksData.values()):
            if 'gridcount' in datObj:#if adding action
                hbox= weeksGrid.itemAt(datObj['gridcount']).layout()
                for i in range(hbox.count()):
                    if isinstance(hbox.itemAt(i).widget(), QDoubleSpinBox):
                        spinbox= QDoubleSpinBox()
                        spinbox.setValue(hbox.itemAt(i).widget().value())
                        # datObj["spinbox"]=hbox.itemAt(i).widget().value()
                        hbox.itemAt(i).widget().deleteLater()
                        datObj["spinbox"]=spinbox
                    elif isinstance(hbox.itemAt(i).widget(), QTextEdit):
                        textedit=QTextEdit()
                        textedit.setText(hbox.itemAt(i).widget().toPlainText())
                        # datObj["textedit"]=hbox.itemAt(i).widget().toPlainText()
                        hbox.itemAt(i).widget().deleteLater()
        ...
        for datObj in list(weeksData.values()):
            ...
            hbox.addStretch(15)
            datObj['gridcount']= weeksGrid.count()
            weeksGrid.addLayout(hbox, i//2, i%2) 
            i+=1
        # adjustSize()
        # update()
        #adjust window size
        if WeekBox.checkedItems:
            ...
            # print("\n")
            # print(len(WeekBox.checkedItems), len(WeekBox.checkedItems)//2)
    ...
    def splitHours(HoursRequiredBox, MinutesRequiredBox, weeksData):
        try:
            ...
            evenhour= totalhours/len(weeksData)
            ...
            for datObj in list(weeksData.values()):
                datObj["spinbox"].setValue(evenhour)
        except:
            MsgBox(str(format_exc())+"\n\n Contact software programmer", setWindowTitle="Error", setIcon = QMessageBox.Critical)
    ...
    def funcOK(self, close=True):
        #sort checked items by date which is gotten from itemData (date is QDate)
        # self.WeekBox.checkedItems.sort(key=lambda x: datetime.strptime(x,"%d/%m/%Y"))
        # print("\n")
        # list(map(lambda x: print(self.WeekBox.itemData(x)), self.WeekBox.checkedItems))
        # print(sorted([self.WeekBox.itemData(x) for x in self.WeekBox.checkedItems]))
        # print(self.weeksData)
        # weekdata={week.toString("dd/MM/yyyy"): {"hrs": dat["spinbox"].value(), "notes": dat["textedit"].toPlainText()} for week, dat in self.weeksData.items()}
        # print(weekdata)
        try:
            actionupdated=False
            ...
            if self.ActionforBox.currentText()!= '' and self.ProjectBox.currentText()!='' and self.ActionEdit.toPlainText()!='':
                ...
                if self.WeekCheck.isChecked():
                    if self.weeksData=={} and len(self.WeekBox.checkedItems)==1: #if only one week is checked, add the full hours to that week
                        weekdata={self.WeekBox.itemData(next(iter(self.WeekBox.checkedItems))).toString("dd/MM/yyyy"): {"hrs": self.HoursRequiredBox.value() + (self.MinutesRequiredBox.value()/60), "notes": ""}}
                    else:
                        weekdata={week.toString("dd/MM/yyyy"): {"hrs": dat["spinbox"].value(), "notes": dat["textedit"].toPlainText()} for week, dat in self.weeksData.items()}
                else:
                    weekdata={}
                ...
                if actionupdated==True:
                    ...
                    if close ==True:
                        ...
                        previous_checkeditems= self.WeekBox.checkedItems
                        ...
                        for dat in newactiondialog.weeksData.values():
                            dat.pop('gridcount')
                        for index in previous_checkeditems:
                            newactiondialog.WeekBox.model().item(index).setCheckState(Qt.Checked)
                        newactiondialog.WeekBox.checkedItems=previous_checkeditems
    ...
    def __init__(self):
        try:
          ...
          #Fetch json data
            ...
            #if app was just opened, set this_userdata to json data for data with matching device
            if 'this_userdata' not in [name for name in globals().keys()]:
                jstopened=True
            else:
                jstopened=False
            global this_userdata  #because you want to change it, do not move this 
            if jstopened:
                this_userdata=None
                for usr in json_data["employees"]:
                    if usr["device"]==thisdevice_name:
                        this_userdata=usr
                        break
            #update data user (incase anything has been changed)
            else:
                if this_userdata!=None:
                    for usr in json_data["employees"]:
                        if usr["device"]==this_userdata['device']:
                            this_userdata=usr
                            break
                    else:
                        this_userdata=None #if user not found , set this_userdata to None
          ...
          #Add projects and actions widgets to the page
            ...
            self.actionsWidget.previous_button.clicked.connect(lambda: self.actionWeekChanged(-7))
            self.actionsWidget.next_button.clicked.connect(lambda: self.actionWeekChanged(7))
...
class NewProject_Dialog(QDialog):
    def __init__(self):
        try:
            ...
            # self.projectClient.move(40,190)
            ...
            # self.projectClientBox.move(140,190)
            
            self.projectArchitects= QLabel('Architects:',self)
            ...
            # self.projectLocation.move(40, 240)
            ...
            # self.projectStatus.move(40,290)
            ...
            # self.projectRole= QLabel('Role:', self)

            # self.projectRIBA= QLabel('Starting RIBA Stage:', self)
          ...
          #Layout
            ...
            self.form.addRow(self.Letter3Code,self.Letter3CodeEdit)
            self.form.addRow(self.projectNo,self.projectNoEdit)
            self.form.addRow(self.projectClient, self.projectClientBox)

            self.form.addRow(self.projectArchitects,self.projectArchitectsBox)
            ...
            if self.projectNameEdit.text()!= '':
                ...
                for files in listdir(Project_Folders_dst):
                    ...
                    if projectExists=="Yes":
                        createProject=False
                        qm = QMessageBox
                        ret = qm.question(self,'RCDC',"The project '"+ self.projectNameEdit.text() +"' already exists in the database\n\nDo you wish to continue and create a new project with same name (different project number)", qm.Yes | qm.No | qm.Cancel)
                        if ret == qm.Yes:
                            createProject=True
                    if createProject==True:
                        ...
                        if createProject==True:
                            ...
                            copytree(TemplateFolder, self.NewProject_Folder)
                            ...
                            #Populute the project details in the project list
                            con_string = r'DRIVER={Microsoft Access Driver (*.mdb, *.accdb)};DBQ='+Central_Database_accdb+';'
                            conn = pyodbc_connect(con_string)
                            cursor =conn.cursor()                    
                            values = (
                                (self.projectNoEdit.text(), self.projectNameEdit.text(), self.Letter3CodeEdit.text() ,self.projectClientBox.currentText(),self.projectStatusBox.currentText(),self.projectArchitectsBox.currentText())
                            )
                            cursor.execute("INSERT INTO ProjectList (ProjectNo, ProjectName, ProjectCode, Client, Status, Architect) VALUES (?,?,?,?,?,?)", values)
                            conn.commit()
                            cursor.close()
                            conn.close()
...
class Management_Window(QMainWindow):
    def __init__(self):
        try:
           ...
           #Users Table
            ...
            self.usersTable.setMaximumWidth(1000)
           ...
           #General defaults
            ...
            self.defaultsForm.setSpacing(25)
            # self.defaultsForm.setContentsMargins(100,0,1400,0)

           #Home Layout
            self.homeOptionsLayout=QHBoxLayout()
            self.homeOptionsLayout.addStretch(8)
            self.homeOptionsLayout.addWidget(self.NewUserButton)
            self.homeOptionsLayout.addStretch(5)
            self.homeOptionsLayout.addWidget(self.RefreshButton)
            # self.homeOptionsLayout.addStretch(5)
            # self.homeOptionsLayout.addWidget(self.ActionApproverButton)
            self.homeOptionsLayout.addStretch(30)

            self.homeTab=QWidget()
            self.homelayout= QVBoxLayout()
            self.homelayout.addStretch(1)
            self.homelayout.addLayout(self.homeOptionsLayout)
            self.homelayout.addStretch(1)
            ...
            self.homeTab.setStyleSheet("""*[mandatoryField="true"] {background:rgb(215, 215, 215);}""")
          ...
          #Holidays page
           #Widgets
            ...
            self.HolidayNameCheck= QCheckBox(text="Name")
            self.HolidayNameEdit=QLineEdit()
            self.HolidayNameEdit.setEnabled(False)
            self.HolidayNameCheck.stateChanged.connect(lambda: self.HolidayNameEdit.setEnabled(self.HolidayNameCheck.isChecked()))
            ...
            self.holidayInfo.setStyleSheet("background:#fcfcfc; padding: 6px;")
            # self.holidayInfo.setMaximumWidth(800)
            # self.holidayInfo.setEnabled(False)

            self.form= QFormLayout()
            self.form.addRow("Who", self.HolidayForBox)
            self.form.addRow(self.HolidayNameCheck, self.HolidayNameEdit)
            self.form.addRow("Start Date", self.StartDateEdit)
            self.form.addRow("End Date", self.EndDateEdit)
            ...
            self.buttonBox =QDialogButtonBox()
            self.CreateHolidayButton=QPushButton("Create Holiday")
            self.CreateHolidayButton.setEnabled(False)
            self.CreateHolidayButton.clicked.connect(self.CreateHolidayClicked)
            self.buttonBox.addButton(self.CreateHolidayButton,QDialogButtonBox.AcceptRole)
           ...
           #Holidays Table
            ...
            self.holidaysTable=QTableWidget(len(self.AllHolidays),7)   
            #Setting header of table
            self.holidaysTable.setHorizontalHeaderLabels(["For","Name","Date","No. of days","Notes","StartDate","Status"])
            
            for i in range(len(self.AllHolidays)):
                # self.item.setFlags(Qt.NoItemFlags)
                # self.holidaysTable.setVerticalHeaderItem(i,self.item)   
                self.holidaysTable.setItem(i,0,QTableWidgetItem(self.AllHolidays[i]["for"]))
                # self.holidaysTable.setItem(self.rowcount,0,QTableWidgetItem(who))
                # self.holidaysTable.item(self.rowcount,0).setFlags((Qt.ItemIsUserCheckable|Qt.ItemIsEnabled))
                self.holidaysTable.setItem(i,1,QTableWidgetItem(self.AllHolidays[i]["name"]))
                
                # self.holidaysTable.item(i,1).setData(256,self.AllHolidays[i]["name"])
                
                self.holidaysTable.setItem(i,2,QTableWidgetItem(self.formatHolidayDates(self.AllHolidays[i])))
                self.holidaysTable.item(i,2).setData(256,self.AllHolidays[i]['start'])
                self.holidaysTable.item(i,2).setData(257,self.AllHolidays[i]['end'])
                # self.holidaysTable.item(i,2).setTextAlignment(Qt.AlignCenter)
                #added to sort date
                self.holidaysTable.setItem(i,5,QTableWidgetItem(""))
                self.holidaysTable.item(i,5).setData(Qt.DisplayRole,QDate(int(self.AllHolidays[i]['start'].split("/")[2]),int(self.AllHolidays[i]['start'].split("/")[1]),int(self.AllHolidays[i]['start'].split("/")[0])))
                self.holidaysTable.setItem(i,3,QTableWidgetItem(str(self.AllHolidays[i]['daysTaken'])))
                self.holidaysTable.item(i,3).setTextAlignment(Qt.AlignCenter)
                self.holidaysTable.setItem(i,4,QTableWidgetItem(self.AllHolidays[i]['notes']))
                self.holidaysTable.item(i,4).setTextAlignment(Qt.AlignCenter)
                self.holidaysTable.setItem(i,6,QTableWidgetItem(self.AllHolidays[i]['status']))
                ...
                if self.AllHolidays[i]["status"]=="unapproved":
                    ...
                    self.holidaysTable.item(i,0).setData(256,"unapproved")
            ...
            self.holidaysTable.setColumnHidden(0,True)
            self.holidaysTable.setColumnHidden(5,True)
            ...
            self.holidaysTable.sortByColumn(2,Qt.AscendingOrder)
            self.holidaysTable.verticalHeader().setHidden(True)

            self.holidaysTable.setColumnWidth(1,190)
            self.holidaysTable.setColumnWidth(2,235)
            ...
            self.holidaysTable.sortByColumn(6, Qt.AscendingOrder)
            ...
            self.lLayout.addWidget(self.formGroupBox)
            self.lLayout.addWidget(self.buttonBox)
           #Calendar widget
            ...
            for hol in self.getAllHolidays():
                ...
                if hol["start"] == hol["end"]:
                    ...
                    if "FirstDayPM" in hol["halfday"]:
                        ...
                        if QDate.fromString(hol["start"],"dd/MM/yyyy") in self.calendarWidget.holidays:
                            ...
                            self.calendarWidget.holidays[QDate.fromString(hol["start"],"dd/MM/yyyy")]=[(hol["for"],"")]
                    self.i=QDate.fromString(hol["start"], "dd/MM/yyyy").addDays(1) #next day
                    #loop until *before* last day
                    while self.i!=QDate.fromString(hol["end"], "dd/MM/yyyy"):
                        if self.i in self.calendarWidget.holidays:
                            self.calendarWidget.holidays[self.i].append((hol["for"],""))
                        else:
                            self.calendarWidget.holidays[self.i]=[(hol["for"],"")]
                        self.i=self.i.addDays(1) #increase day
                    #check for last day
                    if "LastDayAM" in  hol["halfday"]:
           ...
           #Calendar legend
            ...
            legend_frame.setStyleSheet("background-color:white;")
            legend_frame.setMaximumHeight(40)
            legend_frame.setMaximumWidth(600)

            # Create a layout for the legend frame
            legend_layout = QHBoxLayout()
            colours={'Public Holidays':"#bcbfc1",this_userdata['user']: "#b4f4e4","Others":"#fff77d","Clash with more than 1 person":"#f4bccc"}
            ...
            self.lLayout.addWidget(self.calendarWidget)
            # Add the legend layout to the legend frame
            legend_frame.setLayout(legend_layout)
            self.lLayout.addWidget(legend_frame)
            #holidays table and buttons

            self.OptionsLayout=QHBoxLayout()
            self.OptionsLayout.addStretch(1)
            self.OptionsLayout.addWidget(self.RefreshButton2)
            self.OptionsLayout.addStretch(2)

            self.OptionsLayout.addWidget(self.yearBox)
            self.OptionsLayout.addStretch(2)
            self.OptionsLayout.addWidget(self.BankHolidaysButton)
            self.OptionsLayout.addStretch(1)

            self.rLayout=QVBoxLayout()
            self.rLayout.addLayout(self.OptionsLayout)
            self.rLayout.addWidget(self.holidaysTable)
            self.holidaysTab=QWidget()
            self.holidayslayout=QHBoxLayout()
            self.holidayslayout.addLayout(self.lLayout)
            self.holidayslayout.addLayout(self.rLayout)
            self.holidaysTab.setLayout(self.holidayslayout)
            ...
            if event.type()==QEvent.ContextMenu and source==self.usersTable:
                if len(source.selectionModel().selectedRows())>0:
                    ...
                    if len(source.selectionModel().selectedRows())==1:
                        editUserAction = QAction('Edit', self)
                        self.menu.addAction(editUserAction)
                        editUserAction.triggered.connect(lambda: self.editUser())
                    deleteUserAction = QAction('Delete', self)
                    self.menu.addAction(deleteUserAction)
                    deleteUserAction.triggered.connect(lambda: self.deleteUser())
                    self.menu.exec_(event.globalPos())
                    return True
            elif event.type() == QEvent.ContextMenu and source ==self.holidaysTable:
                if len(source.selectionModel().selectedRows())>0:
                    self.menu = QMenu(self)
                    if 'unapproved' in [source.item(i.row(),0).data(256) for i in (source.selectionModel().selectedRows())]:
                        approveAction = QAction(QIcon(tickdone_icon2),"Approve", self)
                        self.menu.addAction(approveAction)
                        approveAction.triggered.connect(lambda: self.approveHolidays())
                    deleteAction = QAction('Delete', self)
                    self.menu.addAction(deleteAction)
                    deleteAction.triggered.connect(lambda: self.deleteHolidays())
                    self.menu.exec_(event.globalPos())
                    return True
            return super().eventFilter(source, event)
        except :
    ...
    # def changeActionApprover(self):
    #     try:
    #         if path.exists(management_json):
    #             item, okPressed = QInputDialog.getItem(self, "Change action approver","Who would you like to assign?\n\nNote: You won't be able to approve actions once you assign a new user", [usr["initial"] for usr in json_data["employees"] if usr["device"]!=json_data["actionapprover"]], 0, False,Qt.WindowCloseButtonHint)
    #             if okPressed and item:
    #                 json_data["actionapprover"]= [usr["device"] for usr in json_data["employees"] if usr["initial"]==item][0]
    ...
    #                 msg= TimerMsgBox("Action approver changed        ",setWindowTitle=" ",setIcon=None, setIconPixmap=tickdone_icon, setWindowIcon=blankimage_icon)       
    #                 msg.exec_()
    #         else:
    #             MsgBox("management.json file is missing\n\nContact software programmer",setWindowTitle="management.json missing", setIcon = QMessageBox.Critical)
    #     except:
    ...
    # def RemoveUserClicked(self):
    #     try:
    #         for j in self.userWidgets:
    #             self.userWidgets[j][1].setVisible(True)
    #     except:
    #         MsgBox(str(format_exc())+"\n\n Contact software programmer", setWindowTitle="Error", setIcon = QMessageBox.Critical)
    ...
    def approveHolidays(self):
        try:
            ...
            hol_rows=[i.row() for i in (self.holidaysTable.selectionModel().selectedRows()) if self.holidaysTable.item(i.row(),0).data(256)=='unapproved']
            self.hols_toapprove="Approve the following "+str(len(hol_rows))+" holidays?\n\n"
            for r in hol_rows:
                self.hols_toapprove+=self.holidaysTable.item(r,0).text()+": "+self.holidaysTable.item(r,1).text()+" "+self.holidaysTable.item(r,2).text()+"\n"
            if hol_rows!=[]:
                qm = QMessageBox
                ...
                if ret==qm.Yes:
                    ...
                    for r in hol_rows: #for each selected pending holiday
                        initial=self.holidaysTable.item(r,0).text() #get initial of user
                        for usr in json_data['employees']: #look for dict of user in json_data['employees']
                            if usr['initial']==initial:
                                ...
                                for hl in usr['holidays'][:]: # look for this holiday and update
                                    if (hl["name"]==self.holidaysTable.item(r,1).text()
                                        and hl["start"]==self.holidaysTable.item(r,2).data(256)
                                        and hl["end"]==self.holidaysTable.item(r,2).data(257)):
                                        hl['status']='approved'
                                        break
                                break
    ...
    def deleteHolidays(self):
        try:
            ...
            if len(self.selectionindexes):
                ...
                for i in self.selectionindexes:
                    if self.holidaysTable.item(i,0).text() not in jsons_toupdate:
                        jsons_toupdate.append(self.holidaysTable.item(i,0).text())
                qm=QMessageBox(self)
                ...
                if qm.clickedButton() in [dontadddays,adddays]:
                    ...
                    for usr in jsons_toupdate:
                        ...
                        for r in self.selectionindexes: # for each holidays to be deleted
                            if self.holidaysTable.item(r,0).text()==usr: # if the holiday is for this current usr
    ...
    def displayHolidayForinYear(self):
        try:
            for row in range(self.holidaysTable.rowCount()):
                if self.holidaysTable.item(row,0).text()!=self.HolidayForBox.currentText()  or self.holidaysTable.item(row,2).data(256)[-4:]!=self.yearBox.currentText():
            ...
            if path.exists(management_json):
                ...
                if okPressed and item:
                    ...
                    for holiday,name in country_holidays('UK',subdiv='England',years=[int(item)+i for i in range(4)]).items():
                        ...
                        for holiday,name in country_holidays('UK',subdiv='England',years=[int(item)+i for i in range(4)]).items():
                            if holiday.weekday()<5: #weekday 0-4 mon-fri
                                json_data["generalholidays"].append({"name":name,"start":datetime.strftime(datetime(year=holiday.year,month=holiday.month,day=holiday.day),"%d/%m/%Y"),"end":datetime.strftime(datetime(year=holiday.year,month=holiday.month,day=holiday.day),"%d/%m/%Y"),"halfday":[], "daysTaken":1, "notes":"", "for":"General","status":"approved"})
                            else:
                                json_data["generalholidays"].append({"name":name,"start":datetime.strftime(datetime(year=holiday.year,month=holiday.month,day=holiday.day),"%d/%m/%Y"),"end":datetime.strftime(datetime(year=holiday.year,month=holiday.month,day=holiday.day),"%d/%m/%Y"),"halfday":[], "daysTaken":0, "notes":"", "for":"General","status":"approved"})
    ...
    def validateDateTime(self):
        try:
            ...
            if self.StartDateEdit.date().dayOfWeek()>5 or self.EndDateEdit.date().dayOfWeek()>5:
                ...
                if self.yearBox.count():
                    ...
                    if self.HolidayForBox.currentText()!="General":  # if this new holiday is for a user, add all holidays for the user
                        ...
                        for holiday in [i['holidays'] for i in json_data['employees'] if i['initial']==self.HolidayForBox.currentText()][0]:
                            self.existingHolidays.append(holiday)
                    else: #if it's a General holiday, check that an existing user holiday doesn't overlap
                    ...
                    if self.proceed:
                        self.holidaysInBetween=list() #Holidays in between this holiday(would only be General Holidays)
                        for holiday in self.existingHolidays:
                        ...
                        # Check if a holiday falls in between this holiday
                            if self.isThisHolidayinBetween(holiday):
                                ...
                                if holiday["for"]=="General":
                                    self.holidaysInBetween.append(holiday)
                                else:
                                    self.CreateHolidayButton.setEnabled(False)
                            ...
                            self.daysTaken=self.StartDateEdit.date().daysTo(self.EndDateEdit.date())+1
                            #Deduct half days
                            ...
                            #Deduct weekends
                            self.i=0
                            while self.StartDateEdit.date().addDays(self.i)!=self.EndDateEdit.date():
                                if self.StartDateEdit.date().addDays(self.i).dayOfWeek()>5:# if day is a week day
                                    self.daysTaken-=1
                                self.i+=1
                            #Deduct any holiday booked in between
                            for holiday in self.holidaysInBetween:
                                self.daysTaken-=holiday["daysTaken"]
                            self.DaysBox.setText(str(self.daysTaken))
                            # holidayinfo=""
                            self.hasEnoughSlots=True
                            ...
                            if self.HolidayForBox.currentText()=="General":
                                ...
                                self.CreateHolidayButton.setEnabled(True)
                            else:
                                #get user's dict
                                self.usr= [i for i in json_data["employees"] if i['initial']==self.HolidayForBox.currentText()][0]
                                self.remainingAnnualHolidays=float(self.usr["remainingannualholidays"])
                                self.remainingCarriedHolidays=float(self.usr["remainingcarriedholidays"])
                                #check year 
                                ...
                                if int(self.yearBox.currentText())==date.today().year:
                                    ...
                                    if self.EndDateEdit.date().toPyDate()<=datetime.strptime(json_data["carriedholidaysdue"]+"/"+str(date.today().year),"%d/%b/%Y").date():
                                        ...
                                        if (self.remainingAnnualHolidays+self.remainingCarriedHolidays)>= self.daysTaken:
                                            if self.remainingCarriedHolidays>=self.daysTaken:
                                                ...
                                                self.holidayInfo.setText(str(self.remainingAnnualHolidays)+" days left after you book this")
                                        else:
                                            self.hasEnoughSlots=False
                                    ...
                                    if self.yearBox.currentText() in self.usr["comingyearsholidaystaken"]:
                                        #check if days taken  and the existing days taken isn't greater than annual holidays limit
                                        if float(self.usr["annualholidays"])>=(self.daysTaken+float(self.usr["comingyearsholidaystaken"][self.yearBox.currentText()])):
                                            self.holidayInfo.setText(str(float(self.usr["annualholidays"])-(self.daysTaken+float(self.usr["comingyearsholidaystaken"][self.yearBox.currentText()])))+" days left after you book this")
                                        else:
                                            self.hasEnoughSlots=False
                                    else:
                                        #if days taken is less than annual holidays allowed
                                        if self.daysTaken<=float(self.usr["annualholidays"]):
                                            self.holidayInfo.setText(str(float(self.usr["annualholidays"])-self.daysTaken)+" days left after you book this")
                                        else:
                                            self.hasEnoughSlots=False
                                if self.hasEnoughSlots:
                                    self.CreateHolidayButton.setEnabled(True)
                                else:
            ...
            # #                     while self.StartDateEdit.date().addDays(self.i)!=self.EndDateEdit.date():
            # #                         if self.StartDateEdit.date().addDays(self.i).dayOfWeek()<6:# if day is a week day
        ...
        try:
            ...
            if not (self.StartDateEdit.text()==self.EndDateEdit.text() and self.FirstDayPMCheck.isChecked() and self.LastDayAMCheck.isChecked()):
                ...
                if self.HolidayForBox.currentText()=="General":
                    if path.exists(management_json):
                        global json_data
                        json_data["generalholidays"].append({"name":self.HolidayNameEdit.text(),"start":self.StartDateEdit.text(),"end":self.EndDateEdit.text(),"halfday":self.halfdaysList,"daysTaken":self.daysTaken,"notes":self.NotesEdit.toPlainText(), "for":self.HolidayForBox.currentText(), "status":"approved"})
                        with open(management_json,'w') as f:
                            json_dump({x: json_data[x] for x in json_data if x !="employees"}, f, indent=2)
                    ...
                    if path.exists(users_jsons+"\\"+self.usr["device"]+"-WOW.json"):
                        ...
                        if int(self.yearBox.currentText())==date.today().year:
                            ...
                            self.usr['holidays'].append({"name":self.HolidayNameEdit.text(),"start":self.StartDateEdit.text(),"end":self.EndDateEdit.text(),"halfday":self.halfdaysList,"daysTaken":self.daysTaken,"notes":self.NotesEdit.toPlainText(),"for":self.HolidayForBox.currentText(),"invoice":{"carriedtaken":float(self.usr["remainingcarriedholidays"])-self.remainingCarriedHolidays,"annualtaken":float(self.usr["remainingannualholidays"])-self.remainingAnnualHolidays},"status":"unapproved"})
                            #update user's remaining holidays
                            self.usr["remainingannualholidays"]=self.remainingAnnualHolidays
                            self.usr["remainingcarriedholidays"]=self.remainingCarriedHolidays
                            ...
                            self.usr['holidays'].append({"name":self.HolidayNameEdit.text(),"start":self.StartDateEdit.text(),"end":self.EndDateEdit.text(),"halfday":self.halfdaysList,"daysTaken":self.daysTaken,"notes":self.NotesEdit.toPlainText(),"for":self.HolidayForBox.currentText(),"invoice":{"carriedtaken":0,"annualtaken":self.daysTaken},"status":"unapproved"})
                            #if user hasn't booked any holiday in the selected year before, add year to comingyearsholidaystaken dict with 0 as value
                            if self.yearBox.currentText() not in self.usr["comingyearsholidaystaken"]:
                                self.usr["comingyearsholidaystaken"][self.yearBox.currentText()]=0
                            #add daysTaken for this holiday to comingyearsholidaysttaken for selected year
                            self.usr["comingyearsholidaystaken"][self.yearBox.currentText()]+=self.daysTaken
                        with open(users_jsons+"\\"+self.usr["device"]+"-WOW.json",'w') as f:
                            json_dump({x: self.usr[x] for x in self.usr if x not in ["actions","password"]},f,indent=2)
                            # json_dump({x: self.usr[x] for x in self.usr if x not in ["actions","actions_toapprove"]},f,indent=2)

                            # json_dump(self.usr, f, indent=2)
                        self.isholAdded=True
                    else:
                        MsgBox(users_jsons+"\\"+self.usr["device"]+"-WOW.json file not found\n\nContact sofware programmer pls", setWindowTitle=users_jsons+"\\"+self.usr["device"]+"-WOW.json missing", setIcon = QMessageBox.Critical)
                if self.isholAdded:
                    holidayfor=managementwindow.HolidayForBox.currentText()
                    widget.removeWidget(managementwindow)
                    managementwindow.__init__()
    ...
    def __init__(self):
        try:
          ...
          #Buttons for project menu options
            ...
            # self.actionTable.doubleClicked.connect(self.on_actionDblclick)
            ...
            if path.exists(ProjectFinanceFolder+'\\3 Fee Proposal'):
                ...
                for file in listdir(ProjectFinanceFolder+'\\3 Fee Proposal'):
                    if file.endswith(".docx"):
                        ...
                        d+=1
                self.FeesTable.doubleClicked.connect(lambda: self.openfile(self.FeesTable))
        ...
        try:
            if event.type() == QEvent.ContextMenu and source ==self.FeesTable:
                if len(source.selectionModel().selectedRows())>0:
                    ...
                    openAction.triggered.connect(lambda: self.openfile(source))
                    self.menu.exec_(event.globalPos())
                    return True
            return super().eventFilter(source, event)
        except :
    ...
    def __init__(self):
        try: 
           ...
           #Groupbox layout
            ...
            self.Frameworks.setStyleSheet("""QGroupBox{ font: 13px;  font-weight: bold; border:1px solid rgba(0, 0, 0, .3); opacity:0.4; margin-top: 0.8em;} 
                                            QGroupBox::title {top: -10px;left: 10px;}""")
                            ...
                            if len(ProjectFinanceFolder+'\\3 Fee Proposal\\'+self.ReportTitle.text()+'.docx')<=259:
                                ...
                                if self.proceed=="Yes":
                                    ...
                                    for par in doc.paragraphs:
                                        if '[SERVICE]' in par.text:
                                            # par.text=par.text.replace('[SERVICE]',self.ReportTitle.text())
                                            inline = par.runs
                                            # Loop added to work with runs (strings with same style)
                                            for i in range(len(inline)):
                                                if '[SERVICE]' in inline[i].text:
                                                    text = inline[i].text.replace('[SERVICE]', self.title)
                                                    inline[i].text = text
                                        if '[PROJECT NAME]' in par.text:
                                            inline = par.runs
                                            for i in range(len(inline)):
                                                if '[PROJECT NAME]' in inline[i].text:
                                                    text = inline[i].text.replace('[PROJECT NAME]', ThisProject_name)
                                                    inline[i].text = text
                                        if '[DD/MM/YYYY]' in par.text:
                                            inline = par.runs
                                            for i in range(len(inline)):
                                                if '[DD/MM/YYYY]' in inline[i].text:
                                                    text = inline[i].text.replace('[DD/MM/YYYY]', datetime.strptime(self.IssueDateEdit.text(), '%Y/%m/%d').strftime('%d/%m/%Y'))
                                                    inline[i].text = text
                                        if '[CLIENT NAME]' in par.text:
                                            inline = par.runs
                                            for i in range(len(inline)):
                                                if '[CLIENT NAME]' in inline[i].text:
                                                    text = inline[i].text.replace('[CLIENT NAME]', ThisProject_client)
                                                    inline[i].text = text
                                        if "[CLIENT LOGO]" in par.text and self.ThisClientLogo!="":
                                            par.text =""
                                            r=par.add_run()
                                            try:
                                    ...
                                    for section in doc.sections:
                                        for par in section.footer.paragraphs:
                                            for r in par.runs:
                                                if r.text=="[PROJECT NAME]":
                                                    r.text=ThisProject_name
                                                elif "[SERVICE]" in r.text:
                                                    r.text=r.text.replace("[SERVICE]",self.title)
        ...
        try:
          ...
          #Get drawings and sketches
            if path.exists(Project_Database): #Check if the path exists, if it does, get Drawings and Sketches
                self.DrawingsTable.doubleClicked.connect(lambda: self.openfile(self.DrawingsTable))
                self.Drawings_search.textChanged.connect(lambda: self.filter_search(self.DrawingsTable,self.Drawings_search))
                self.SketchesTable.doubleClicked.connect(lambda: self.openfile(self.SketchesTable))
        ...
        try:
            ...
            if event.type() == QEvent.ContextMenu and source in [self.DrawingsTable, self.SketchesTable]:
                if len(source.selectionModel().selectedRows())>0:
                    ...
                    SScopy = QAction('Copy to SS')
                    menu.addAction(deleteAction)
                    menu.addAction(openAction)
                    menu.addAction(SScopy)
                    deleteAction.triggered.connect(lambda: self.delete(source))
                    openAction.triggered.connect(lambda: self.openfile(source))
                    SScopy.triggered.connect(lambda:self.copytoSS(source))
                    menu.exec_(event.globalPos())
                    # print(event)
                    # print(source)
                    return True
        ...
        try:
           ...
           #get background list from backgrounds folder
            ...
            if path.exists(ProjectDrawingFolder+'\\2 Backgrounds')==True: #Check if the path exists
                self.BackgroundList.doubleClicked.connect(lambda: self.openfile(self.BackgroundList))
                        ...
                        for self.drawing in range(self.drawingcopies_Table.rowCount()): #Ensure the drawing you're creating doesn't already exist
                            ...
                            for self.drawing in range(self.drawingcopies_Table.rowCount()):
                                ...
                                if self.drawingcopies_Table.cellWidget(self.drawing, 1).isChecked():
                                    editPDF(ProjectDrawingFolder+'\\2 Backgrounds\\'+XrefsDict[self.drawingcopies_Table.item(self.drawing, 0).text()]+".pdf",ProjectDrawingFolder+'\\'+self.subfolder +'\\Stage '+RIBAstages[ThisProject_no+' - '+ ThisProject_name]+'\\'+self.drawingcopies_Table.item(self.drawing,0).text()+".pdf",textBoxDict={"ProjectName":self.ProjectNameEdit.text(),"ProjectNo":ThisProject_no,"Client":self.ProjectClientBox.currentText(), "Architect": self.ProjectArchitectBox.currentText(),"Designby":self.DesignbyBox.currentText(), "Drwn1":self.DesignbyBox.currentText(), "Service":role, "DrawingTitle":str(self.drawingcopies_Table.item(self.drawing,0).text().split(" - ",1)[-1]), "DrawingNo":(self.drawingcopies_Table.item(self.drawing,0).text().split(" - ",1))[0]})
                                else:
                                    if drawingwindow.tableSelected==drawingwindow.SketchesTable:
                                        if self.A3RadioBut.isChecked()==True:
                                            editPDF(SketchA3_Temp,ProjectDrawingFolder+'\\'+self.subfolder +'\\Stage '+RIBAstages[ThisProject_no+' - '+ ThisProject_name]+'\\'+self.drawingcopies_Table.item(self.drawing,0).text()+".pdf",textBoxDict={"ProjectName":self.ProjectNameEdit.text(),"Designby":self.DesignbyBox.currentText(), "Drwn1":self.DesignbyBox.currentText(),"ProjectNo":ThisProject_no, "DrawingTitle":str(self.drawingcopies_Table.item(self.drawing,0).text().split(" - ",1)[-1]), "DrawingNo":(self.drawingcopies_Table.item(self.drawing,0).text().split(" - ",1))[0],"DateCreated":self.DateCreatedEdit.text()})
                                        else:
                                            editPDF(SketchA1_Temp,ProjectDrawingFolder+'\\'+self.subfolder +'\\Stage '+RIBAstages[ThisProject_no+' - '+ ThisProject_name]+'\\'+self.drawingcopies_Table.item(self.drawing,0).text()+".pdf",textBoxDict={"ProjectName":self.ProjectNameEdit.text(),"Designby":self.DesignbyBox.currentText(), "Drwn1":self.DesignbyBox.currentText(),"ProjectNo":ThisProject_no, "DrawingTitle":str(self.drawingcopies_Table.item(self.drawing,0).text().split(" - ",1)[-1]), "DrawingNo":(self.drawingcopies_Table.item(self.drawing,0).text().split(" - ",1))[0],"DateCreated":self.DateCreatedEdit.text()})
                                    else:
                                        editPDF(DrawingA1_Temp,ProjectDrawingFolder+'\\'+self.subfolder +'\\Stage '+RIBAstages[ThisProject_no+' - '+ ThisProject_name]+'\\'+self.drawingcopies_Table.item(self.drawing,0).text()+".pdf",textBoxDict={"ProjectName":self.ProjectNameEdit.text(),"ProjectNo":ThisProject_no,"Client":self.ProjectClientBox.currentText(), "Architect": self.ProjectArchitectBox.currentText(),"Designby":self.DesignbyBox.currentText(), "Drwn1":self.DesignbyBox.currentText(), "Service":role, "DrawingTitle":str(self.drawingcopies_Table.item(self.drawing,0).text().split(" - ",1)[-1]), "DrawingNo":(self.drawingcopies_Table.item(self.drawing,0).text().split(" - ",1))[0]})
                                
                                values = (
                                ((self.drawingcopies_Table.item(self.drawing,0).text().split(" - ",1))[0][:-2], (self.drawingcopies_Table.item(self.drawing,0).text().split(" - ",1))[0][-2:], self.drawingcopies_Table.item(self.drawing,0).text().split(" - ",1)[-1])
                                )
                                cursor.execute("INSERT INTO "+ self.table +" (Ref, SeqNo, PostName) VALUES (?,?,?)", values)
                            conn.commit()
                            cursor.close()
    ...
    def __init__(self):
        ...
        #     self.CalcTable.setHorizontalHeaderLabels(["","Calc Number","Calc Name","Created by"])
            ...
            if path.exists(Project_Database) and path.exists(ProjectCalcsFolder):
                ...
                self.CalcTable.doubleClicked.connect(self.tbl_double_clicked)
            ...
            if event.type() == QEvent.ContextMenu and source in [self.CalcTable]:
                ...
                if len(selectedRows)>0 :
                    ...
                    if all(source.item(i,source.columnLabels.index('Type')).text()=='File' for i in selectedRows):
                        openAction = QAction('Open')
                        menu.addAction(openAction)
                        openAction.triggered.connect(source.openfile)
                    menu.addAction(deleteAction)
                    deleteAction.triggered.connect(lambda: self.delete(source))
                    menu.exec_(event.globalPos())
                    return True
            ...
            if len(selectionindexes)>0:
                ...
                if ret == qm.Yes:
                    if tableobj == self.CalcTable: #Check for selected items Table to the database to delete from 
                        self.CalcTable.deletefiles() #delete actual files first
                        ...
                        for i in selectionindexes:
                            #if the selected item is a file, delete it from the database
                            filename= tableobj.item(i,tableobj.columnLabels.index('Name')).text()
                            if tableobj.item(i,tableobj.columnLabels.index('Type')).text()=='Folder':
                                subfiles= self.CalcTable.getsubfiles(self.CalcTable.cur_dir+'\\'+filename)
                                for file in subfiles: #continue from here
                                    if file[-5:-1]=='.xls' and file[:-5]  in calcsAccessnames:
                                        dbfilestoDelete.append(file[:-5])
                            if tableobj.item(i,tableobj.columnLabels.index('Type')).text()=='File' and filename[-5:-1]=='.xls' and filename[:-5]  in calcsAccessnames:
                                dbfilestoDelete.append(filename[:-5])
                        if dbfilestoDelete:
                            con_string = r'DRIVER={Microsoft Access Driver (*.mdb, *.accdb)};DBQ='+Project_Database+';' #Connect with the access database
                            conn = pyodbc_connect(con_string)
                            cursor =conn.cursor()            
                            for d in dbfilestoDelete: # for each items, delete records from access
                                cursor.execute("DELETE FROM Calcs WHERE (FullName = '"+d+ "')")
                                # cursor.execute("DELETE FROM Calcs WHERE (PreName = '"+ str(tableobj.item(i,1).text()) + "' AND PostName = '"+ str(tableobj.item(i,2).text()) + "')")
...
class NewCalc_Dialog(QDialog):
    ...
    def setNameBox(self):
        try: 
            self.editNameButton.setEnabled(True)
            serviceno=self.CalcTemplateListBox.currentItem().data(258)
            self.prename=project3name_dict[ThisProject_foldername]+'-RCDC-'+RIBAstages[ThisProject_no+' - '+ ThisProject_name]+'-XX-CALC-'+serviceno
            if self.prename in CalcSeqNoDict: #check for the number in the calc seqno dictionary
                count = CalcSeqNoDict[self.prename]+1
            else:
                count= 1
            # self.prename+='-'+self.sequential_no
            self.nameBox.setText(self.CalcTemplateListBox.currentItem().text())

            while path.exists(ProjectCalcsFolder+"\\Stage "+RIBAstages[ThisProject_no+' - '+ ThisProject_name]+"\\"+self.prename+'-'+str(count).zfill(4)+' - '+self.nameBox.text()+'.xlsx'):
                count+=1  
        ...
        try:
           ...
           #get templates list from calcs template folder
            ...
            if path.exists(CalcsTempFolder): #Check if the path exists
                self.TemplatesList.doubleClicked.connect(lambda: self.openfile(self.TemplatesList))
            ...
            if event.type() == QEvent.ContextMenu and source in [self.DrawingsTable,self.IssuesFolderTable]:
                ...
                if len(source.selectionModel().selectedRows())>0:
                    ...
                    if source==self.DrawingsTable:
                        openAction = QAction('Open')
                        menu.addAction(openAction)
                        deleteAction.triggered.connect(self.deleteDrawings)
                        openAction.triggered.connect(self.openDrawings)
                    elif source==self.IssuesFolderTable:
                        deleteAction.triggered.connect(self.deleteFolder)
                        if len(source.selectionModel().selectedRows())==1:
                            opensheetAction = QAction('View Issue Sheet')
                            opensheetAction.triggered.connect(self.OpenIssueSheet)
                            menu.addAction(opensheetAction)
...
class NewIssue_Dialog(QDialog):
    def __init__(self, docsDict=None):
        try:
          ...
          #Get drawings and sketches
            if path.exists(Project_Database): #Check if the path exists, if it does, get Drawings and Sketches
                self.DrawingsTable.doubleClicked.connect(lambda: self.expand_or_fold(self.DrawingsTable))
                self.SketchesTable.doubleClicked.connect(lambda: self.expand_or_fold(self.SketchesTable))
                con_string = r'DRIVER={Microsoft Access Driver (*.mdb, *.accdb)};DBQ='+Project_Database+';'
                conn = pyodbc_connect(con_string)
                cursor =conn.cursor()  
                ...
                if self.issueRecords!=None:
                    ...
                    for doc in self.docs:
                        ...
                        if ref not in [rec['Ref'] for rec in self.records]:
                            name=path.splitext(fileName)[0].split(' - ')[1]
    ...
    def eventFilter(self, source, event): 
        try:
            if event.type() == QEvent.ContextMenu and source.__class__ in [QLabel]:
                ...
                self.menu.addAction("Open", lambda: self.openFile(source))
                self.menu.exec_(event.globalPos())
                return True
            return super().eventFilter(source, event)
        except:
    ...
    def __init__(self):
        try:
          ...
          #Presentations Table
            ...
            # self.PresentationsTable.doubleClicked.connect(lambda: self.openfile(self.PresentationsTable,'5 Presentations'))
          ...
          #Memos Table
            ...
            # self.MemosTable.doubleClicked.connect(lambda: self.openfile(self.MemosTable,'0 Sketches'))
          ...
          #Get reports
            if path.exists(Project_Database):
                ...
                self.ReportsTable.doubleClicked.connect(lambda: self.openfile(self.ReportsTable))
...
class NewReport_Dialog(QDialog):
    ...
    def RoleChanged(self, index):
        try:
            ...
            self.prename = project3name_dict[ThisProject_foldername] + '-RCDC-'+ RIBAstages[ThisProject_no+' - '+ ThisProject_name]+'-XX-RP-'+self.RoleListBox.currentData()
                            ...
                            if path.exists(self.ReportTemp):
                                ...
                                if self.proceed=="Yes":
                                    ...
                                    for section in doc.sections:
                                        for par in section.footer.paragraphs:
                                            for r in par.runs:
                                                if r.text=="Report name":
                                                    r.text=self.ReportTitleEdit.text()
                                                    break
...
class SpecsWindow(QMainWindow):
    def __init__(self):
        try:
          ...
          #Get reports
            if path.exists(Project_Database):
                self.SpecsTable.doubleClicked.connect(lambda: self.openfile(self.SpecsTable))
                self.Specs_search.textChanged.connect(lambda: self.filter_search(self.SpecsTable,self.Specs_search))
                self.SchedulesTable.doubleClicked.connect(lambda: self.openfile(self.SchedulesTable))   
                                    ...
                                    while True:
                                        try:
                                            ...
                                            for par in doc.paragraphs:#Check for Report Title in the doc and replace with the title in the report title box
                                                if par.text=="[ReportTitle]":
                                                    par.add_run().font.size = Pt(48) #set font size
                                                    par.runs[0].text=typ #Replace text
                                                elif par.text=="[ProjectTitle]":##Similar to report title
                                                    par.add_run().font.size = Pt(24) 
                                                    par.runs[0].text=ThisProject_name
                                                    # par.text=ThisProject_name
                                                    ...
                                                    for run in par.runs:
                                                        if "[ProjectName]"  in run.text:
                                                            run.text= run.text.replace("[ProjectName]",ThisProject_name)
                                                elif "[ProjectClient]" in par.text:
                                                    for run in par.runs:
                                                        if "[ProjectClient]"  in run.text:
                                                            run.text= run.text.replace("[ProjectClient]",ThisProject_client)
                                                    ...
                                                    for run in par.runs:
                                                        if "[ReportReference]"  in run.text:
                                                            run.text= run.text.replace("[ReportReference]",specsRef)
                                            #tables
                                            for table in doc.tables:
                                                for row in table.rows:
                                                    for cell in row.cells:  
                                            ...
                                            for foot in range(0, len(doc.sections)):
                                                #In the footers replace all texts 'Report name' with the report name set
                                                # doc.sections[foot].footer.paragraphs[0].text =doc.sections[foot].footer.paragraphs[0].text.replace('Report name', typ)
                                                doc.sections[foot].footer.paragraphs[0].text =doc.sections[foot].footer.paragraphs[0].text.replace('Project title', ThisProject_name)
...
class NewSchedules_Dialog(QDialog):
    ...
    def getRef(self, schdict):
        try:
            prename= project3name_dict[ThisProject_foldername] +'-RCDC-'+RIBAstages[ThisProject_no+' - '+ ThisProject_name]+'-XX-SH-'+schdict['ServiceNo']
            #Check if this name already exists
            ...
            if item:
                ...
                if self.lastCheckedIndex is not None and (self.shiftPressed or self.ctrlShiftPressed or self.ctrlPressed):
                    if self.ctrlPressed: #If only Ctrl is held, check/uncheck current item without affecting previous selections
                        ...
                        start = min(index.row(), self.lastCheckedIndex.row())
                        end = max(index.row(), self.lastCheckedIndex.row())
...
class HolidaysCalendarWidget(QCalendarWidget):
    ...
    def paintCell(self, painter, rect, date):
        ...
        #colours 
        colours={'General':"#bcbfc1","MyHoliday": "#b4f4e4","OthersHoliday":"#fff77d","ClashHolidays":"#f4bccc"}
                ...
                if perd not in holcol:
                    ...
                    if quicklist:
                        if len(quicklist)>1 or (perd in ['LastDayAM','FirstDayPM'] and [h for h in self.holidays[date] if h[1]=='']):#more than one person booked 
                            holcol[perd]=colours['ClashHolidays'] 
                        elif quicklist[0][0]==this_userdata['initial']: #user's holiday
                            holcol[perd]=colours['MyHoliday']
                        else:       # only one person booked but not this user
                            holcol[perd]=colours['OthersHoliday']
...
class FileExplorerTable(QTableWidget):
    def __init__(self, home_dir):
        try:
            ...
            #set selection bg to similat to that of windows explorer
            ...
            # self.doubleClicked.connect(self.tableItemDblClicked)
    ...
    def populate_table(self, directory):
        try:
            ...
            for item in dir_content:
                row = self.rowCount()
                self.insertRow(row)
                self.setItem(row, self.columnLabels.index("Name"), QTableWidgetItem(item.fileName()))
                if item.isDir(): 
                    if item.fileName() == "..":
                        self.setItem(row, self.columnLabels.index("Name"), QTableWidgetItem("Back"))
                ...
                type_item = QTableWidgetItem("Folder" if item.isDir() else "File")
                self.setItem(row, self.columnLabels.index("Type"), type_item)
                if not item.isDir():
                    size_item = QTableWidgetItem(f"{ceil(item.size()/1000):,} KB")
    ...
    def tableItemDblClicked(self):
        try:
            selected_items =[i.row() for i in self.selectionModel().selectedRows(0)]
            if len(selected_items)==1 and self.item(selected_items[0],self.columnLabels.index("Type")).text()=="Folder":
                selectedtext= self.item(selected_items[0],self.columnLabels.index("Name")).text()
                if selectedtext=="Back": #go back to parent directory
                    selectedtext=".."
                elif selectedtext=="Home": #go back to parent directory
                    selectedtext="."
                # print(selectedtext)
                # print(self.cur_dir, selectedtext)
                # print(path.abspath(self.cur_dir+"\\"+selectedtext))
                #prevent going out of parent directory
                if path.isdir(self.cur_dir+"\\"+selectedtext) and path.abspath(self.cur_dir+"\\"+selectedtext)!=path.abspath(self.home_dir+"\\.."):
                    if selectedtext=="." : #go back to parent directory
                        if path.abspath(self.cur_dir)!=path.abspath(self.home_dir):
                            self.populate_table(self.home_dir)   
                    else:                 
                        self.populate_table(path.abspath(self.cur_dir + "\\" + selectedtext)) # don't extract this line
                
            else:
                if all([self.item(i,self.columnLabels.index("Type")).text()=="File" for i in selected_items]):#if all selected items are files
                    self.openfile()
    ...
    def openfile(self):
        try:
            selectionindexes = [i.row() for i in self.selectionModel().selectedRows(0) if self.item(i.row(),self.columnLabels.index('Type')).text() =='File'] #Make a list of selected files
            for i in selectionindexes:
    ...
    def deletefiles(self):
        try:
            selectionindexes = [i.row() for i in self.selectionModel().selectedRows(0) if self.item(i.row(),self.columnLabels.index('Type')).text() =='File'] #Make a list of selected files
            for i in selectionindexes:
                toDelete= self.cur_dir+'\\'+self.item(i,self.columnLabels.index('Name')).text()
                if path.exists(toDelete):
                    while True: # As long as the file is open, keep displaying the msgbox to tell the user to close the file until it is closed( open files have no permission to be deleted)
                        try:
                            if path.isdir(toDelete)==False:
                                remove(toDelete) # delete calc
                            #     rmtree(toDelete)
                            # else:
                            break
                        except IOError: # ioerror is the error encountered if the file isn't able to be deleted because it is open
                            #If there's error while opening the file, tell user to close the file to continue
                            MsgBox("Please make sure the file '"+ toDelete+"' isn't open\n\nClick OK after it is closed",setWindowTitle="File open", setIcon = QMessageBox.Critical)
        except:
            MsgBox(str(format_exc())+"\n\n Contact software programmer", setWindowTitle="Error", setIcon = QMessageBox.Critical)
...
if __name__ == "__main__":
        ...
        try:
            ...
            from os import rename, listdir, path, startfile, remove, environ, walk #, getlogin
            ...
            #fontfile
            gadugi_fontfile= userpath + r"\Ruane Construction Design and Consultancy\RCDC - Documents\Projects\00 - Programming Codes\WoW\DO NOT TOUCH\gadugi-normal.ttf"
